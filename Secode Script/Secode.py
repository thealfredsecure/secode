...
print(head)
Creators = "Created by @AliasgarSabun , @sec_noel , @OorjaRungta and @MonsieurDartag1"
text = Creators.center(100)
print(text)
print("")
print("")
print("Secode version 1.0 (C) 2021")
...
c_ruleset = {
    ...
    #       is that secode doesn't have type information, and ">>" is safe with
    #       many other types.
...
p_directive = re.compile(r'(?i)\s*(ITS4|secode|RATS):\s*([^\*]*)')
...
def display_header():
    ...
    if not displayed_header:
        if output_format:
            ...
            print("<title>secode Results</title>")
            print('<meta name="author" content="--">')
            print('<meta name="keywords" lang="en" content="secode results, security scan">')
            print("</head>")
            print("<body>")
            print("<h1>secode Results</h1>")
            print("Here are the security scan results from")
            print('(C) 2021 <a href="</a>.')
        else:
            print("secode version %s, (C) 2021" % version)
        displayed_header = 1
...
def process_file_args(files, patch_infos):
    ...
    # (i.e. through recursing into a directory) because secode
    ...
    for f in files:
        if (not allowlink) and os.path.islink(f):
            ...
            # At one time secode used os.path.walk, but that Python
...
def usage():
    print("""
secode [--help | -h] [--version] [--listrules]
  ...
  The options cover various aspects of secode as follows.
  Documentation:
  ...
  --error-level=LEVEL
              ...
              especially if you mark known-okay lines as "secode: ignore".
              Usually you want level to be fairly high, such as 4 or 5.
              By default, secode returns 0 (true) on a successful run.
  Hitlist Management:
  --savehitlist=F
